#' GIGG for Logistic Regression
#'
#' Fits a Bayesian grouped logistic regression model using the GIGG prior.
#'
...
#' @param ... Additional arguments passed to \code{gigg_gibbs}.
#'
#' @return An object of class \code{gigg} containing:
...
#' @examples
#' \dontrun{
#' fit <- gigg(y ~ ., data = my_data, groups = my_groups)
#' summary(fit)
#' }
gigg <- function(formula, data, groups, n_iter = 20000, burnin = 10000,
  ...
  # 5. Run Gibbs sampler
  if (verbose) {
    cat("\nRunning GIGG Gibbs sampler...\n")
  }

  start_time <- Sys.time()

  posterior <- gigg_gibbs(
    data = data_gibbs,
    nsamples = n_iter,
    burnin = burnin,
    init = init,
...
#' Create Initial Values for GIGG
#'
#' Internal function to create initial values for the GIGG sampler.
#'
#' @noRd
